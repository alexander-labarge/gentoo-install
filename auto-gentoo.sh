...
genkernel --kernel-append-localversion=-labarge-intel-i7-optimized-12feb24 --mountboot --microcode initramfs --install all
...
#!/bin/bash
echo deathstar-bridge > /etc/hostname
...
# Variables for user password hashes
root_hash='$y$j9T$UN9CqzL.SU/hsjEQl1UeQ.$Kha121fdD3WNfIekv9mHpcXDUsiM4ksbMxq3ywa3d1C'
skywalker_hash='$y$j9T$UN9CqzL.SU/hsjEQl1UeQ.$Kha121fdD3WNfIekv9mHpcXDUsiM4ksbMxq3ywa3d1C'

# Update /mnt/gentoo/etc/shadow file for root and skywalker
SHADOW_FILE="/mnt/gentoo/etc/shadow"

# Update root's password hash
sed -i "s|^root:[^:]*|root:${root_hash}|" "${SHADOW_FILE}"

# Update skywalker's password hash
sed -i "s|^skywalker:[^:]*|skywalker:${skywalker_hash}|" "${SHADOW_FILE}"

echo "/etc/shadow has been updated for root and user: skywalker. hint = c 2x"
...
ln -sf "/mnt/gentoo/usr/lib/systemd/system/sshd.service" "/mnt/gentoo/etc/systemd/system/multi-user.target.wants/sshd.service"
